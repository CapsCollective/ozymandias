...
float4 TriplanarSampling(TEXTURE2D_PARAM(Texture, Sampler), float3 Position, float3 Normal, float Blend, float2 Tile)
{
    float3 projNormal = (pow(abs(Normal), Blend));
    projNormal /= (projNormal.x + projNormal.y + projNormal.z) + 0.00001f;
    float3 nsign = sign(Normal);
    half4 xNorm; half4 yNorm; half4 zNorm;
    xNorm = SAMPLE_TEXTURE2D(Texture, Sampler, Tile * Position.zy * float2(nsign.x, 1.0f));
    yNorm = SAMPLE_TEXTURE2D(Texture, Sampler, Tile * Position.xz * float2(nsign.y, 1.0f));
    zNorm = SAMPLE_TEXTURE2D(Texture, Sampler, Tile * Position.xy * float2(-nsign.z, 1.0f));
...
#endif
{
    ...
    BRDFData brdfData;
    InitializeBRDFData(GetTerrainColor(inputData.positionWS, inputData.normalWS), metallic, /* specular */ half3(0.0h, 0.0h, 0.0h), smoothness, alpha, brdfData);
